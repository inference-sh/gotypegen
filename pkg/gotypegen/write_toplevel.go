...
// isEmitVar returns true if dec is a string var with a gotypegen:emit directive.
func (g *PackageGenerator) isEmitVar(dec *ast.GenDecl) bool {
	...
	for _, c := range dec.Doc.List {
		if strings.HasPrefix(c.Text, "//gotypegen:emit") {
...
// emitVar emits the text associated with dec, which is assumed to be a string var with a
// gotypegen:emit directive, as tested by isEmitVar.
